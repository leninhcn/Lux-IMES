<template>
  <div class="upload-file">
    <el-upload
      ...
      ref="fileUpload">
      <!-- 拖拽上传 -->
      <template v-if="drag" class="custom-upload">
        <div>
      ...
      <!-- 上传按钮 -->
      <el-button type="primary" icon="UploadFilled" v-if="!drag">选取文件</el-button>
      <!-- 上传提示 -->
      <template v-slot:tip>
        <div class="el-upload__tip" v-if="showTip">
          <slot name="tip">
            ...
            <template v-if="limit">
              , 最多文件个数 <b class="text-danger">{{ limit }}个</b>
            </template>
          </slot>
        </div>
      </template>
...
const props = defineProps({
  ...
  fileType: {
    type: Array,
    default: () => ['doc', 'xls', 'ppt', 'txt', 'pdf', 'jpg', 'png', 'tgz']
  },
  // 是否显示提示
  isShowTip: {
    type: Boolean,
...
const uploadFileUrl = ref(baseUrl + '//business/NpiProjet/UploadFile') // 上传的图片服务器地址
...
function handleUploadError(err) {
  proxy.$modal.msgError(`上传失败，handleUploadError原因:${err}!`)
  // console.log(err)
  // failedUploads.value++
...
function uploadChange(file) {
  const allowedTypes = ['doc', 'xls', 'ppt', 'pptx', 'txt', 'pdf', 'jpg', 'png', 'xlsx', 'tgz']
  const maxSize = 1024 * 1024 * 50 // 5MB，可以根据需要调整
  ...
  if (!isAllowed) {
    ...
    proxy.$modal.msgError('文件格式不正确，请上传以下格式doc', 'xls', 'ppt', 'txt', 'pdf', 'jpg', 'png', 'xlsx')
    return
  }
  if (!isWithinSizeLimit) {
    ...
    fileUpload.value.clearFiles()
    //console.log(fileList.value)
    proxy.$modal.msgError('文件太大超过50M，请上传标准的文件格式！')
    return
  }
}
...
.custom-upload {
  width: 100px; /* 设置宽度 */
  height: 10px; /* 设置高度 */
  //   border: 2px dashed #409eff; /* 设置边框样式 */
  //   border-radius: 5px; /* 设置边框圆角 */
  //   display: flex; /* 设置为弹性布局以垂直居中内容 */
  //   flex-direction: row; /* 设置内容垂直排列 */
  //   justify-content: center; /* 在垂直方向上居中内容 */
  //   align-items: center; /* 在水平方向上居中内容 */
  //   cursor: pointer; /* 设置光标样式为指针 */
}
</style>
