			...
			<el-row item-class="mt-1" h="full" mt="-3" :gutter="10">
				<el-col :span="12" h="full">
					<div h="full" class="block-bar" b-rd="2" px="4" py="4">
						<Stack size="" h="full" max-w="100" truncate>
							<Stack horizontal>
								<el-text>QC In 站点：</el-text>
								<div item-flex="1" item-nogap></div>
								<el-select v-model="state.qcInStation" w="70">
									<el-option v-for="(item, index) in vc.qcInStationList" :key="index" :label="item"
										:value="item"></el-option>
								</el-select>
							</Stack>
							<Stack horizontal>
								<el-text>QC Out 站点：</el-text>
								<div item-flex="1" item-nogap></div>
								<el-select v-model="state.qcOutStation" w="70">
									<el-option v-for="(item, index) in vc.qcOutStationList" :key="index" :label="item"
										:value="item"></el-option>
								</el-select>
							</Stack>
							<Stack horizontal>
								<el-text>输入类型：</el-text>
								<div item-flex="1" item-nogap></div>
								<el-select v-model="state.inputType" w="70">
									<el-option v-for="(item, index) in vc.inputTypes" :key="index" :label="item"
										:value="item"></el-option>
								</el-select>
							</Stack>
						</Stack>
				...
				<el-col :span="12" h="full">
					<el-row h="full" class="block-bar" b-rd="2" py="2" pl="8" truncate>
						<el-col :span="18" h="full">
							<Stack size="small" h="full" py="10" px="10">
								<Stack horizontal size="small" flex-items="center">
									<el-text>工单</el-text>
									<div item-flex="1"></div>
									<el-text item-flex="0 0 80%">{{ state.curWo }}</el-text>
								</Stack>
								<Stack horizontal size="small">
									<el-text>料号</el-text>
									<div item-flex="1"></div>
				...
				<el-col :span="8" h="full">
					<div h="full" truncate class="block-bar" b-rd="2" p="4">
						<Stack size="small" h="full" overflow="auto">
							<InputBox label="条码" ref="snInput" v-model.trim="state.input" :readonly="viewState.input.readonly"
								v-model:disabled="viewState.input.disabled" v-model:focused="viewState.input.focused"
								@submit="vc.onInputSubmit"/>
						</Stack>
					</div>
				</el-col>
