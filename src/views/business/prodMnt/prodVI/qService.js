	...
	if (res.ok) {
		...
		switch (label) {
			case 'SN':
				...
				snInfo.mpn = data['SN_GET_MPN'].toString()
				snInfo.regionCode = data['SN_GET_REGION_CODE'].toString()
				snInfo.imei = data['SN_GET_IMEI'].toString()
				snInfo.imei2 = data['SN_GET_IMEI2'].toString()
				snInfo.moped = data['SN_GET_MOPED'].toString()
				break
			case "WO":
				if (!data) return
...
export async function checkLogic(station, step, errorCode) {
	const result = { ok: true, msg: '' }
