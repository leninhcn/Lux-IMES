<template>
  <div class="app-container" v-loading="loading" element-loading-text="进维修..." element-loading-spinner="el-icon-loading">
    ...
    <Station-Config @changeStation="changeStation" ref="stationConfig" storageKey="checkInStation" :op-types="['Repair']" :client-types="['MANU']"></Station-Config>
  </div>
</template>
<script setup>
import msg from '@/plugins/modal'
import { requestRepairIn } from '@/api/repair'

// 站点配置
const stationConfig = ref(null)
const str = localStorage.getItem('checkInStation')
...
async function repairIn() {
  ...
  try {
    loading.value = true
    const res = await requestRepairIn({ lblType: 'SN', sn: form.value.sn, ...stationInfo.value })
    if (res.code === 200) {
        result.value = res
        msg.msgSuccess('操作成功')
    }
