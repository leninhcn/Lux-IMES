...
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['business:psnstatus:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
    ...
    <el-table
      ...
      <el-table-column prop="workOrder" label="WorkOrder" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('workOrder')"/>
      <el-table-column prop="model" label="Model" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('model')"/>
      <el-table-column prop="serialNumber" label="SerialNumber" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('serialNumber')"/>
      ...
      <el-table-column prop="routeName" label="RouteName" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('routeName')"/>
      ...
      <el-table-column prop="stationType" label="StationType" align="center" v-if="columns.showColumn('stationType')">
      ...
      <el-table-column prop="inStationtypeTime" label="InStationtypeTime" :show-overflow-tooltip="true"  v-if="columns.showColumn('inStationtypeTime')"/>
      <el-table-column prop="outStationtypeTime" label="OutStationtypeTime" :show-overflow-tooltip="true"  v-if="columns.showColumn('outStationtypeTime')"/>
      ...
      <el-table-column prop="machineNo" label="MachineNo" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('machineNo')"/>
      <el-table-column prop="toolingNo" label="ToolingNo" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('toolingNo')"/>
      ...
      <el-table-column label="操作" width="160">
        <template #default="scope">
          <el-button type="success" size="small" icon="edit" title="编辑" v-hasPermi="['business:psnstatus:edit']" @click="handleUpdate(scope.row)"></el-button>
          <el-button type="danger" size="small" icon="delete" title="删除" v-hasPermi="['business:psnstatus:delete']" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="psnstatus">
import { listPSnStatus,
 addPSnStatus, delPSnStatus,
 updatePSnStatus,getPSnStatus,
 }
from '@/api/business/psnstatus.js'
...
const columns = ref([
  { visible: true, prop: 'snVersion', label: 'SnVersion' },
  { visible: true, prop: 'locationNo', label: 'LocationNo' },
  { visible: true, prop: 'createTime', label: 'CreateTime' },
  { visible: true, prop: 'oPTION1', label: 'OPTION1' },
  { visible: true, prop: 'workOrder', label: 'WorkOrder' },
  { visible: true, prop: 'model', label: 'Model' },
  { visible: true, prop: 'serialNumber', label: 'SerialNumber' },
  { visible: true, prop: 'ipn', label: 'Ipn' },
  { visible: false, prop: 'version', label: 'Version' },
  { visible: false, prop: 'routeName', label: 'RouteName' },
  { visible: false, prop: 'line', label: 'Line' },
  { visible: false, prop: 'stage', label: 'Stage' },
  { visible: false, prop: 'stationType', label: 'StationType' },
  ...
  { visible: false, prop: 'inStationtypeTime', label: 'InStationtypeTime' },
  { visible: false, prop: 'outStationtypeTime', label: 'OutStationtypeTime' },
  ...
  { visible: false, prop: 'machineNo', label: 'MachineNo' },
  { visible: false, prop: 'toolingNo', label: 'ToolingNo' },
...
function getList(){
  loading.value = true
  listPSnStatus(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
const state = reactive({
  ...
  form: {},
  rules: {
    serialNumber: [{ required: true, message: "SerialNumber不能为空", trigger: "blur" }],
  },
  options: {
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.id != undefined && opertype.value === 2) {
        updatePSnStatus(form.value).then((res) => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addPSnStatus(form.value).then((res) => {
