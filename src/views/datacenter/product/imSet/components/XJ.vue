<template>
  ...
  <el-dialog v-model="dialogVisible.open" :title="dialogVisible.title" width="500" draggable>
    <el-form ref="dialogFormRef" :model="dialogVisible.form" :rules="dialogVisible.rules" label-width="100">
        ...
        <el-form-item :label="$t('imSet.machine')" prop="machine">
            <el-input v-model="dialogVisible.form.machine" clearable :placeholder="$t('common.inputTip')" />
        </el-form-item>
        <el-form-item :label="$t('imSet.tooling')" prop="tooling">
            <el-input v-model="dialogVisible.form.tooling" clearable :placeholder="$t('common.inputTip')" />
        </el-form-item>
        <el-form-item :label="$t('imSet.cavity')" prop="cavity">
            <el-input v-model="dialogVisible.form.cavity" clearable :placeholder="$t('common.inputTip')" />
        </el-form-item>
  ...
  import { getImsetDataXJ, addImsetData, editImsetData, delImsetData, getImsetLog,getModelData,getLineData,getStageData,getStationData,getSJVendorData,getBuildData } from '@/api/datacenter/product/mimSet'
  ...
  const columns = reactive([
    ...
    { key: 4, label: proxy.$t('imSet.machine'), prop: 'machine', visible: true, sortable: true },
    { key: 5, label: proxy.$t('imSet.tooling'), prop: 'tooling', visible: true, sortable: true },
    { key: 5, label: proxy.$t('imSet.cavity'), prop: 'cavity', visible: true, sortable: true },
  ...
  const options = reactive([
    ...
    {
      label: proxy.$t('imSet.machine'),
      prop: 'machine'
    },
    {
      label: proxy.$t('imSet.tooling'),
      prop: 'tooling'
    },
    {
      label: proxy.$t('imSet.cavity'),
      prop: 'cavity'
    }
  ])
  ...
  const getTableData = async () => {
    loading.value = true
    try {
      const res = await getImsetDataXJ({ ...formData.value, pageNum: currentPage.value, pageSize: pageSize.value })
  ...
  const dialogVisible = reactive({
    ...
    rules: {
        ...
        machine: [{ required: true, message: proxy.$t('common.inputTip'), trigger: 'blur' }],
        tooling: [{ required: true, message: proxy.$t('common.inputTip'), trigger: 'blur' }],
        cavity: [{ required: true, message: proxy.$t('common.inputTip'), trigger: 'blur' }]
    }
  })
  ...
  const dialogConfirm = async (formEl) => {
    ...
    await formEl.validate(async (valid, fields) => {
      ...
      let res
      try {
        dialogVisible.form.sxl_type = "XL"
  ...
  const showLog = async ({ id,model_name }) => {
    logDialogVisible.title = `下料 ${model_name} ${proxy.$t('btn.log')}`
  ...
  // 导出
  const dowloadData = () => {
    proxy.downFile('/datacenter/imset/ImSetExportXJ', { pageSize: total.value })
  }
  </script>
