...
// 被拖拽的元素
const dragItem = ref()
const dragChildrenItem = ref()

// 被拖拽的数据
const draggableData = ref({})

// 拖拽的是整体(entirety)还是单个(single)
let draggableType = 'entirety'

// 被拖拽的索引
const dragIndex = ref(0)

// 被拖拽元素 鼠标按下
const downItem = (data, i, e) => {
  dragIndex.value = i
  draggableData.value = data
  if (e) {
    draggableType = 'single'
    e.target.style.backgroundColor = '#a0cfff'
  } else {
    draggableType = 'entirety'
...
const listMouseup = () => {
  ...
  let params = {}
  if (draggableType === 'single') {
    const data = props.list2.find((item) => item.stage === props.list1[dragIndex.value].stage)
    if (data === undefined) {
      params = { ...props.list1[dragIndex.value] }
      params.children = [draggableData.value]
    } else {
      let isHas = false
      for (let index = 0; index < data.children.length; index++) {
        if (data.children[index].stationType === draggableData.value.stationType) {
          isHas = true
        }
      }
      if (isHas) {
      ...
      data.children.push(draggableData.value)
      params = data
    }
  } else {
    const data = props.list2.find((item) => item.stage === draggableData.value.stage)
    if (data && data.children.length === draggableData.value.children.length) {
      return proxy.$modal.msgWarning(proxy.$t('common.already'))
    }
    params = draggableData.value
  }
  emit('updateList2', 'drag', params)
  open.value = true
  draggableData.value = {}
}
...
const upItem = () => {
  if (draggableType == 'single') {
    for (let i = 0; i < dragChildrenItem.value.length; i++) {
      dragChildrenItem.value[i].style.backgroundColor = 'transparent'
    }
  } else {
...
const handlemenuItemClick = async (type) => {
  if (type === 'add') {
    ...
    // 删除  当没有第三层数据的时候父级数据点击删除会报错 得由后端操作，前端操作太过麻烦！！！
        ...
        <div ref="dragItem" class="item drag-item" v-for="(item, i) in list1" :key="item.stage" @mousedown="downItem(item, i)">
          ...
          <div class="children one" v-show="item.open">
            <span v-for="val in item.children" :key="val.stationType" :style="childrenStyle(val)">
              ...
              <span ref="dragChildrenItem" @mousedown.stop="downItem(val, i, $event)">{{ val.stationType }} {{ val.stationTypesDesc }}</span>
            </span>
          </div>
        </div>
      </el-scrollbar>
