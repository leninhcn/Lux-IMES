...
const updateList2 = (type, data, row) => {
  ...
  // 拖拽
  if (type === 'drag') {
    list2.value = list2.value.filter((item) => item.stage !== arr.stage)
    for (let i = 0; i < arr.children.length; i++) {
      arr.children[i] = {
        stationType: arr.children[i].stationType,
        stationTypesDesc: arr.children[i].stationTypesDesc,
        children: arr.children[i].children || [],
        open: false
      }
    }
    arr.open = true
