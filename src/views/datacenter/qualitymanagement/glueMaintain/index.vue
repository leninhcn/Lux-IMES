<script setup name="gluemaintain">
...
import {
  ...
  editModelData,
  delModelData,
  editDescriptionData, lineList,
} from '@/api/datacenter/qualitymanagement/glueMaintain'
import {ElMessageBox} from 'element-plus'
import {getToken} from '@/utils/auth'
import TableDialog from "@/views/datacenter/qualitymanagement/glueMaintain/components/tableDialog.vue";
...
const upload = reactive({
  ...
  url: import.meta.env.VITE_APP_BASE_API + '/ProdMnt/CallPart/CallPartImport'
})
...
const importTemplate = () => {
  proxy.downFile('/ProdMnt/CallPart/ShowExport', {pageSize: total.value})
}
// 导出
const dowloadData = () => {
  proxy.downFile('/ProdMnt/CallPart/ShowExport', {pageSize: total.value})
...
<template>
  ...
  <ITable :columns="columns" :tableData="tableData" :loading="loading" :handleSelectionChange="handleSelectionChange">
    <!-- 操纵按钮 -->
    <template #tool-btns>
      <el-button type="primary" plain icon="Plus" v-hasPermi="['datacenter:qualitymanagement:glueMaintain:add']"
      ...
      <el-button
        ...
        plain
        icon="Edit"
        v-hasPermi="['datacenter:qualitymanagement:glueMaintain:edit']"
      ...
      <el-button
        ...
        v-hasPermi="['datacenter:qualitymanagement:glueMaintain:enable']"
        :disabled="selectData.length != 1"
      ...
      <el-button
        ...
        plain
        icon="Delete"
        v-hasPermi="['datacenter:qualitymanagement:glueMaintain:remove']"
        v-if="formData.enaBled === 'N'"
        :disabled="selectData.length != 1"
        @click="delData(selectData[0])"
      >{{ $t('btn.delete') }}
      </el-button>
      <el-button type="info" plain icon="Upload" v-hasPermi="['datacenter:qualitymanagement:glueMaintain:import']"
                 @click="uploadData">{{
          $t('btn.import')
        }}
      </el-button>
      <el-button type="warning" plain icon="Download" v-hasPermi="['datacenter:qualitymanagement:glueMaintain:export']"
                 @click="dowloadData">{{
          $t('btn.export')
        }}
      </el-button>
    ...
    <template #operation="{ row }">
      ...
      <el-button type="primary" size="small" icon="Reading" text
                 v-hasPermi="['datacenter:qualitymanagement:gluemaintain:history']"
                 @click="showLog(3,row)">
        {{ $t('查看历史') }}
      </el-button>
    </template>
