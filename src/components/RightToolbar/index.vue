<template>
  <div class="top-right-btn">
    <el-tooltip v-if="ellipsis" class="item" effect="dark" :content="tableWhole ? $t('common.tableEllipsis') : $t('common.tableWhole')"
      placement="top">
      <el-button circle icon="MoreFilled" @click="toggleTableDisplay()" />
    </el-tooltip>

    <el-tooltip class="item" effect="dark" :content="$t('common.isShowCol')" placement="top" v-if="data">
      <el-dropdown :hide-on-click="false" style="margin-left: 10px;margin-right: 10px;" trigger="click">
    ...
    <el-tooltip class="item" effect="dark" :content="showSearch ? $t('common.hiddenSearch') : $t('common.showSearch')" placement="top">
...
const emits = defineEmits(['update:columns', 'update:showSearch', 'queryTable'])
...
const tableWhole = ref(false)

// 缩略
function toggleTableDisplay() {
  tableWhole.value = !tableWhole.value
  if (!props.columns) return
  emits('update:columns', props.columns.map(e => {
    e.showTooltip = !tableWhole.value
    return e
  }))
}
